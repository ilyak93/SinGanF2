...
def train(opt, Gs, Zs, reals, NoiseAmp):
    ...
    while scale_num < opt.stop_scale + 1:
        ...
        if (nfc_prev == opt.nfc):
            G_prev = torch.load('%s/%d/netG.pth' % (opt.out_, scale_num - 1), map_location=torch.device('cpu'))
            ...
            D_prev = torch.load('%s/%d/netD.pth' % (opt.out_, scale_num - 1), map_location=torch.device('cpu'))
